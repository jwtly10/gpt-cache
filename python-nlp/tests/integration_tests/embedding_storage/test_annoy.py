...
def annoy_storage_and_embeddings():
    ...
    annoy_storage = AnnoyEmbeddingStorage(dimension=384)  # Dimension matches the model's output
    ...
    sentences = [closely_related_sentence1, closely_related_sentence2] + unrelated_sentences
...
def test_find_closely_related_sentence(annoy_storage_and_embeddings):
    annoy_storage, sentence_embedding = annoy_storage_and_embeddings
    query_embedding = sentence_embedding.to_embedding("The quick fox who is brown jumps over the dog who is lazy")
    nearest_neighbors = annoy_storage.get_nns_by_vector(query_embedding, n=1)
    nearest_neighbor_id = nearest_neighbors[0][0]
    assert nearest_neighbor_id == 1, f"Expected the nearest neighbor to be ID 1, got {nearest_neighbor_id}."

    annoy_storage, sentence_embedding = annoy_storage_and_embeddings
    query_embedding = sentence_embedding.to_embedding("The language of the universe is mathematics.")
    nearest_neighbors = annoy_storage.get_nns_by_vector(query_embedding, n=1)
    nearest_neighbor_id = nearest_neighbors[0][0]
    assert nearest_neighbor_id == 5, f"Expected the nearest neighbor to be ID 5, got {nearest_neighbor_id}."